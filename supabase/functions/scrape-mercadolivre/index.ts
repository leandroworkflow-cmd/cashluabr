...
const AFFILIATE_TAG = 'leandro4marketing';
const TARGET_URL = 'https://www.mercadolivre.com.br/ofertas';

function withAffiliate(url: string): string {
  try {
    const u = new URL(url);
    u.searchParams.set('matt_word', AFFILIATE_TAG);
...
Deno.serve(async (req) => {
  ...
  try {
        ...
        ofertas: {
          ...
          items: {
            ...
            properties: {
              ...
              preco_original: { type: 'string', description: 'Preço antes do desconto, se houver' },
              desconto: { type: 'string', description: 'Percentual de desconto, ex: 30' },
              imagem: { type: 'string', description: 'URL absoluta da imagem do produto' },
              link: { type: 'string', description: 'URL absoluta do produto no Mercado Livre' },
            },
        ...
        formats: [
          {
            type: 'json',
            schema,
            prompt: 'Extraia todas as ofertas/promoções visíveis na página com título, preço atual, preço original (se houver), percentual de desconto, URL absoluta da imagem e URL absoluta do produto.',
          },
        ],
    ...
    console.error('scrape-mercadolivre error', err);
